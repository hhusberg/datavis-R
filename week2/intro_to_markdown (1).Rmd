...
Add a new chunk by clicking the *Insert Chunk* button on the toolbar or by pressing *Ctrl+Alt+I*. You can give a name to the chunk (this is useful when knitting Markdown documents).  
...
**Difference between Markdown and Notebook**: unlike *Knit*, *Preview* does not run any R code chunks. Instead, the output of the chunk when it was last run in the editor is displayed. When knitting, it is checked that everything you need to run the code successfully is included in the .Rmd file. For this reason, a new R session is started. See what happens if you replace `html_notebook` with `html_document` in the metadata (and save the document)! There are also some differences in how the output looks (for example tables).  
