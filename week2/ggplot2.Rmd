...
Plotting with ggplot2 consists of components (functions) joined by `+`:

- initialise the plot with `ggplot()`: what data do you want to plot?
  - specify aesthetics with `aes()`: by default, the first two will be x and y variables
...
Aesthetics (plotted variables) don't need to be specified in the `ggplot()` component (since that's not the component that draws the points), but can be defined within the geom. However, this can lead to repetitive code when using multiple geoms.   
