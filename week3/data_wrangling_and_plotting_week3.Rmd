...
You can find out your **working directory** (i.e. the folder you are operating in) with the `getwd()` command. In R Markdown chunks, the working directory defaults to the folder where the .Rmd file is located.
...
```{r}
setwd('C:/Users/tuisku.tammi/Documents/GitHub/datavis-R') # change this to match your own computer's directories
...
```{r}
setwd('C:/Users/tuisku.tammi/Documents/GitHub/datavis-R')

list.files() # lists files of the current working directory

list.files('week3') # or a sub-directory

list.files('C:/Users/tuisku.tammi/Documents/GitHub/datavis-R') # or a specific full path

list.files('C:/Users/tuisku.tammi/Documents/GitHub/datavis-R', full.names = T) # full.names = T for full path

```

Using `full.names = T` gives you the complete path to the files. You *can* use the full name of the path to read files, but don't need to, if the beginning of the path is your working directory. For example, if your data file ('data.txt') is located in *'C:/Users/user.name/Documents/R_course/week3'*, and your working directory is set to *'C:/Users/user.name/Documents/R_course'*, you would only need to add *'week3/data.txt'* when you read the file. Also, **using full paths makes it difficult to open the files on another computer!**.

#### Projects

Using projects helps with all this hassle. When you open an .R or .Rmd file from a project, your working directory is restored to the project folder, and all you need is in one place. Later, e.g. when saving figures, this is very useful.  
...
```{r}
df <- readr::read_csv('region_scores.csv')
...
guess_encoding('region_scores.csv')
...
Let's see what happens when we cut the **score** variable with each of these functions. The original range of score values is `r range(df$score)`.  
...
```{r}
df_stats %>%
  ggplot(aes(education, mean_age, fill = score_group)) + 
...
```{r}
df %>%
  ggplot(aes(fct_reorder(region, mean_age), mean_age)) + # reordering done within ggplot - be careful with brackets...
  geom_point() + 
  coord_flip()
  
```

This code produces the same output while using the raw age variable in reordering and plotting. We haven't specified a summary function to stat_summary, and it will use "mean_se" by default.     
