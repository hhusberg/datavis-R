---
title: "Converting between long and wide data"
...
```
We can specify the desired column names.
...
In fact, this data frame is a special case because we're gathering all existing columns and therefore don't need to worry about extra columns. Usually, you should specify the columns that should be gathered.
...
df_long
```
The result is a long data frame which *repeats the values of participant and gender for each session*. When the data is in a long format, we can use the session variable as a grouping variable in plotting, in the same way we're using the grouping variable. On the other hand, the numerical values we want to plot are all in one column.   
...
Now we can calculate the number of weeks that each of the songs were in the charts, as well as the best ranking of each song, and number of weeks spent at #1. 
...
billboard_long %>%
  ...
  select(-week) %>%
  spread(day, rank) %>%
  rename_at(6:9, function(x) paste0("day_", x)) %>%
  arrange(day_3) %>%
...
billboard_long %>%
  ...
  ungroup %>%
  ggplot(aes(week, rank, group = track, colour = interaction(artist, track, sep = ": ", lex.order = T))) + 
  geom_line(alpha=.5, size=1) +
  guides(colour = guide_legend(ncol = 1, title = "Track")) + theme_bw()
