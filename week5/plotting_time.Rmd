...
```{r message=F}
timeuse <- read_csv("timeuse.csv") %>% as_tibble()
...
# method 1: if_else
tidytime_agg %>%
  mutate(is_weekend = if_else(weekday %in% c("Sat", "Sun"), 1, 0)) # if_else(condition, true values, false values)
...
(Function `plot_grid()` from the cowplot package is used to print plots side-by-side.)
...
fig1 <- tidytime %>% # start with "raw" data
  ...
  labs(x = "Weekday", y = "Average time spent (minutes)", colour = "Activity type", title = "Figure 1",
...
fig2 <- tidytime_agg %>% # start with aggregated data
  ...
  ggplot(aes(weekday, time_spent, color = activity_class, group=1)) + geom_line(size=1) + 
  facet_wrap(~activity_class, scales = "free_y") +
  labs(x = "Weekday", y = "Average time spent (minutes)", colour = "Activity type", title = "Figure 2",
